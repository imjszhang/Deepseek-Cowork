...
| 场景 | 示例 |
|------|------|
| 任务委派 | "帮我审查这个模块，总结风险并给出安全的重构方案" |
| 项目初始化 | "创建项目目录结构，并准备初始配置文件" |
| 内容整理 | "把下载文件夹里的文件按类型分类" |
| 会话跟进 | "总结今天改了什么，并提醒我下一步该做什么" |
...
| **LocalService** | 驱动桌面端、Web 界面与 CLI 工作流的本地运行时 |
...
- `npm run local-service` 直接启动当前受支持的 LocalService。
- `npm run server` 仅保留为 `npm run local-service` 的兼容别名。
- `server/index.js` 已废弃，不再作为受支持的独立服务入口。
...
```bash
# 全局安装 CLI 工具（最新版本: 0.3.0）
npm install -g deepseek-cowork@0.3.0

# 或直接从仓库启动同一套 LocalService
...
# 检查 docs/app 是否仍与 renderer 源码同步
...
项目采用语义化版本规范 (SemVer)。当前版本：**V0.3.0**
