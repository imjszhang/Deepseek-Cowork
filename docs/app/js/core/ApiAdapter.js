...
function detectEnvironment() {
    ...
    if ((window.apiAdapter || window.browserControlManager) && typeof (window.apiAdapter || window.browserControlManager) === 'object') {
        // 进一步检查是否有关键方法
        if (typeof (window.apiAdapter || window.browserControlManager).getServerStatus === 'function') {
            return 'electron';
        }
    }
    return 'web';
...
class ApiAdapter {
    ...
    async _callElectron(method, ...args) {
        ...
        if (typeof (window.apiAdapter || window.browserControlManager)[ipcMethod] !== 'function') {
            throw new Error(`Method "${ipcMethod}" not found in browserControlManager`);
        }
        
        return (window.apiAdapter || window.browserControlManager)[ipcMethod](...args);
    }
    ...
    on(event, callback) {
        ...
        if (this._mode === 'electron' && (window.apiAdapter || window.browserControlManager).on) {
            (window.apiAdapter || window.browserControlManager).on(event, callback);
        }
    }
    ...
    off(event, callback) {
        ...
        if (this._mode === 'electron' && (window.apiAdapter || window.browserControlManager).off) {
            (window.apiAdapter || window.browserControlManager).off(event, callback);
        }
    }
