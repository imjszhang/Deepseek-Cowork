...
const DEEPSEEK_MAIN_MODEL = 'deepseek-v4-pro';
const DEEPSEEK_FLASH_MODEL = 'deepseek-v4-flash';
const DEEPSEEK_BASE_URL = 'https://api.deepseek.com/anthropic';
const LEGACY_DEEPSEEK_MAIN_MODELS = new Set(['deepseek-chat', 'deepseek-v4-pro[1m]']);
...
const DEFAULT_SETTINGS = {
    happy: {
        ...
        claudeCode: {
            ...
            defaultOpusModel: null,    // ANTHROPIC_DEFAULT_OPUS_MODEL
            defaultSonnetModel: null,  // ANTHROPIC_DEFAULT_SONNET_MODEL
            defaultHaikuModel: null,   // ANTHROPIC_DEFAULT_HAIKU_MODEL
            subagentModel: null,       // CLAUDE_CODE_SUBAGENT_MODEL
            effortLevel: null,         // CLAUDE_CODE_EFFORT_LEVEL
...
class UserSettingsCLI {
    ...
    _load() {
        try {
            if (fs.existsSync(this._settingsPath)) {
                ...
                const migrated = this._migrateClaudeCodeSettings();
                if (migrated) {
                    this._save();
                }
            } else {
    ...
    /**
     * 迁移旧版 DeepSeek Claude Code 配置到当前默认值
     * @returns {boolean} 是否发生迁移
     * @private
     */
    _migrateClaudeCodeSettings() {
        const claudeCode = this._settings?.happy?.claudeCode;
        if (!claudeCode || claudeCode.provider !== 'deepseek') {
            return false;
        }

        let changed = false;

        const useMainModel = (value) => value === null || value === undefined || LEGACY_DEEPSEEK_MAIN_MODELS.has(value);
        const useFlashModel = (value) => value === null || value === undefined || value === DEEPSEEK_MAIN_MODEL || LEGACY_DEEPSEEK_MAIN_MODELS.has(value);

        if (claudeCode.baseUrl === null || claudeCode.baseUrl === undefined || claudeCode.baseUrl === DEEPSEEK_BASE_URL) {
            if (claudeCode.baseUrl !== DEEPSEEK_BASE_URL) {
                claudeCode.baseUrl = DEEPSEEK_BASE_URL;
                changed = true;
            }
        }

        if (useMainModel(claudeCode.model)) {
            if (claudeCode.model !== DEEPSEEK_MAIN_MODEL) {
                claudeCode.model = DEEPSEEK_MAIN_MODEL;
                changed = true;
            }
        }

        if (useFlashModel(claudeCode.smallFastModel)) {
            if (claudeCode.smallFastModel !== DEEPSEEK_FLASH_MODEL) {
                claudeCode.smallFastModel = DEEPSEEK_FLASH_MODEL;
                changed = true;
            }
        }

        if (useMainModel(claudeCode.defaultOpusModel)) {
            if (claudeCode.defaultOpusModel !== DEEPSEEK_MAIN_MODEL) {
                claudeCode.defaultOpusModel = DEEPSEEK_MAIN_MODEL;
                changed = true;
            }
        }

        if (useMainModel(claudeCode.defaultSonnetModel)) {
            if (claudeCode.defaultSonnetModel !== DEEPSEEK_MAIN_MODEL) {
                claudeCode.defaultSonnetModel = DEEPSEEK_MAIN_MODEL;
                changed = true;
            }
        }

        if (useFlashModel(claudeCode.defaultHaikuModel)) {
            if (claudeCode.defaultHaikuModel !== DEEPSEEK_FLASH_MODEL) {
                claudeCode.defaultHaikuModel = DEEPSEEK_FLASH_MODEL;
                changed = true;
            }
        }

        if (useFlashModel(claudeCode.subagentModel)) {
            if (claudeCode.subagentModel !== DEEPSEEK_FLASH_MODEL) {
                claudeCode.subagentModel = DEEPSEEK_FLASH_MODEL;
                changed = true;
            }
        }

        if (claudeCode.effortLevel === null || claudeCode.effortLevel === undefined) {
