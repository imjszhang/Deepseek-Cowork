    ...
    app.post('/api/account/secret', async (req, res) => {
        try {
            ...
            try {
                ...
                console.log('[API] account/secret - Computed new anonId:', newAnonId);
            } catch (e) {
                console.warn('[API] account/secret - Failed to derive anonId:', e.message);
            }
            
            // 如果 HappyService 已初始化，尝试重启
            if (isAccountSwitching && hasExistingSecret && HappyService.isInitialized()) {
                try {
                    // 传入新的配置，确保 HappyService 使用新账户
                    const newOptions = {
                        happySecret: validation.normalized,
                        serverUrl: userSettings.get('happy.serverUrl') || undefined,
                        anonId: newAnonId
                    };
                    console.log('[API] account/secret - Hot switching with new options');
                    const restartResult = await HappyService.restartDaemon(newOptions);
                    
                    if (restartResult.success) {
                        // 传入 anonId 并检查连接结果
                        const connectResult = await HappyService.connectToSession('main', { anonId: newAnonId });
                        ...
                        if (connectResult.success) {
                            ...
                            console.warn('[API] account/secret - Connection failed after restart:', connectResult.error);
                        }
                    }
                } catch (e) {
            ...
            if (!hasExistingSecret && HappyService.needsLogin()) {
                ...
                try {
                    ...
                    if (startResult.success) {
                        ...
                        // 连接到 session（传入 anonId）
                        await HappyService.connectToSession('main', { anonId: newAnonId });
...
module.exports.syncCredentialsToHappyDir = syncCredentialsToHappyDir;
