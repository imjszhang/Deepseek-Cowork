...
function settingsRoutes(app, context) {
     ...
     * 切换工作目录
     * 
     * 更新顺序（以 userSettings 为权威源）：
     * 1. 保存旧值（用于回滚）
     * 2. 更新 userSettings（权威源）
     * 3. 更新 SessionManager（HappyService.switchWorkDir）
     * 4. 更新 Explorer（热切换监控目录）
     * 5. 任一步骤失败时回滚 userSettings
     */
    app.put('/api/settings/workspace', async (req, res) => {
        try {
            ...
            console.log('[Settings] Switching workspace to:', newPath);
            
            // 确保目录存在
            ensureDir(newPath);
            
            // 1. 保存旧值（用于回滚）
            const oldWorkspaceDir = userSettings.get('happy.workspaceDir');
            
            // 2. 先更新 userSettings（权威源）
            userSettings.set('happy.workspaceDir', newPath);
            console.log('[Settings] userSettings updated');
            
            // 3. 执行 SessionManager 热切换
            const result = await HappyService.switchWorkDir(newPath);
            
            if (!result.success) {
                // SessionManager 切换失败，回滚 userSettings
                console.warn('[Settings] SessionManager sync failed, rolling back userSettings...');
                userSettings.set('happy.workspaceDir', oldWorkspaceDir);
                console.log('[Settings] Rollback complete');
                
                return res.json({
                    success: false,
                    error: result.error || 'Failed to switch session'
                });
            }
            
            console.log('[Settings] SessionManager sync complete');
            
            // 4. 通知 Explorer 服务切换监控目录（热更新）
     ...
     * 更新顺序（以 userSettings 为权威源）：
     * 1. 保存旧值（用于回滚）
     * 2. 更新 userSettings 为 null（使用默认目录）
     * 3. 更新 SessionManager（HappyService.switchWorkDir）
     * 4. 更新 Explorer（热切换监控目录）
     * 5. 任一步骤失败时回滚 userSettings
     */
    app.delete('/api/settings/workspace', async (req, res) => {
        try {
            ...
            console.log('[Settings] Resetting workspace to default:', defaultDir);
            ...
            console.log('[Settings] userSettings reset to null (default)');
            ...
            if (HappyService.isInitialized()) {
                ...
                if (localService?._explorerService) {
                    try {
                        const explorerResult = await localService._explorerService.switchWatchDir(defaultDir);
                        if (explorerResult.success) {
                            console.log('[Settings] Explorer watch directory reset to:', defaultDir);
                        } else {
                            // Explorer 切换失败不回滚，因为核心状态已经同步
                            console.warn('[Settings] Failed to reset explorer watch directory:', explorerResult.error);
                        }
                    } catch (e) {
