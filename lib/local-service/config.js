...
const SERVICE_ROLE = 'local-backend';
...
/**
 * 获取服务实例信息文件路径
 * @returns {string} 服务实例信息文件路径
 */
function getServiceInfoPath() {
    return path.join(getDataDir(), 'service.json');
}
...
/**
 * 写入当前服务实例信息
 * @param {Object} info 服务实例信息
 */
function writeServiceInfo(info) {
    ensureDir(getDataDir());
    const payload = {
        app: APP_NAME,
    ...
    fs.writeFileSync(getServiceInfoPath(), JSON.stringify(payload, null, 2), 'utf8');
}
...
function readServiceInfo() {
    try {
        const infoPath = getServiceInfoPath();
