...
class LocalService extends EventEmitter {
    ...
    async _startSubServices() {
        ...
        // 初始化 Memory 服务
        if (this._config.memory?.enabled !== false) {
            logInfo('Starting Memory service...');
            
            const { setupMemoryService } = require('../../server/modules/memory');
            
            this._memoryService = setupMemoryService({
                serverConfig: {
                    host: this._config.server.host,
                    port: this._config.server.port
                },
                dataDir: path.join(this._dataDir, 'memories')
            });
            
            await this._memoryService.init();
            this._memoryService.setupRoutes(this._app);
            await this._memoryService.start();
            
            // 事件转发
            this._memoryService.on('memory:saved', (data) => {
                this.emit('memory:saved', data);
            });
            
            logInfo('Memory service started');
        }
    }
    ...
    async stop(options = {}) {
        ...
        try {
            ...
            if (this._memoryService) {
                await this._memoryService.stop();
            }
