...
import { l as logger, p as projectPath, f as delay, I as InvalidateSync, R as RawJSONLinesSchema, g as AsyncLock, c as configuration, d as packageJson, e as encodeBase64, h as encodeBase64Url, i as decodeBase64, j as getLocalHappyAgentCredentialPath, r as readLocalHappyAgentCredentials, k as decryptWithDataKey, m as decryptLegacy, n as detectResumeSupport, o as detectCLIAvailability, q as decrypt, A as ApiClient, b as connectionState, s as startOfflineReconnection, t as getLatestDaemonLog } from './types-BDOVRaQz.mjs';
...
import { statSync as statSync$1, existsSync as existsSync$1, writeFileSync as writeFileSync$1, chmodSync, unlinkSync as unlinkSync$1, mkdirSync as mkdirSync$1 } from 'fs';
import { join as join$1 } from 'path';
import 'expo-server-sdk';
import { execSync, execFileSync } from 'node:child_process';
import { readDaemonState, clearDaemonState, readSettings, readCredentials, updateSettings, writeCredentialsLegacy, writeCredentialsDataKey, acquireDaemonLock, readPersistedSessions, persistSession, writeDaemonState, releaseDaemonLock, clearCredentials, clearMachineId, SandboxConfigSchema } from './persistence-tjdFxr4R.mjs';
...
function buildSandboxRuntimeConfig(sandboxConfig, sessionPath) {
  ...
  const allowWrite = (() => {
    switch (sandboxConfig.sessionIsolation) {
      ...
      case "workspace": {
        const workspaceRoot = sandboxConfig.workspaceRoot ? expandPath(sandboxConfig.workspaceRoot, sessionPath) : resolve(sessionPath);
...
async function claudeRemoteLauncher(session) {
  ...
  function onMessage(message) {
    ...
    if (!runtimeErrorNotified && message.type === "system" && (message.error_status || message.error)) {
      ...
      logger.warn("[remote]: runtime system error", {
  ...
  try {
    ...
    while (!exitReason) {
      ...
      try {
        ...
        logger.debug("[remote]: launch error", launchErrorDetails);
        logger.warn("[remote]: launch error details", launchErrorDetails);
        if (!exitReason) {
          session.client.closeClaudeSessionTurn("failed");
          session.client.sendSessionEvent({ type: "message", message: "Process exited unexpectedly" });
          continue;
...
function startCaffeinate() {
  ...
  try {
    caffeinateProcess = spawn$2("caffeinate", ["-im"], {
      stdio: "ignore",
      detached: false
    });
...
function killOrphanedCaffeinateProcesses() {
  ...
  try {
    execSync$1('pkill -f "caffeinate -im"', { timeout: 5e3, stdio: "ignore" });
...
class TmuxUtilities {
  ...
  runCommand(args, options = {}) {
    return new Promise((resolve, reject) => {
      const child = spawn$2(args[0], args.slice(1), {
...
function decryptSessionMetadata(session, credentials) {
  ...
  const metadata = encryption.variant === "dataKey" ? decryptWithDataKey(encryptedMetadata, encryption.key) : decryptLegacy(encryptedMetadata, encryption.key);
  if (!metadata) {
...
async function startDaemon() {
  ...
  try {
      ...
      try {
        ...
        if (options.token) {
          if (options.agent === "codex") {
            ...
            await fs.writeFile(join$1(codexHomeDir.name, "auth.json"), options.token);
    ...
    const bundlePath = join$1(projectPath(), "dist", "index.mjs");
    let initialBundleMtimeMs = 0;
    try {
      initialBundleMtimeMs = statSync$1(bundlePath).mtimeMs;
    } catch {
...
(async () => {
  ...
  if (subcommand === "doctor") {
    ...
    if (geminiSubcommand === "project" && args[2] === "set" && args[3]) {
      ...
      try {
        const { saveGoogleCloudProjectToConfig } = await import('./config-DJ9D4j6h.mjs');
        const { readCredentials: readCredentials2 } = await import('./persistence-tjdFxr4R.mjs');
        const { ApiClient: ApiClient2 } = await import('./types-BDOVRaQz.mjs').then(function (n) { return n.u; });
    ...
    if (geminiSubcommand === "project" && args[2] === "get") {
      try {
        const { readGeminiLocalConfig } = await import('./config-DJ9D4j6h.mjs');
    ...
    try {
      const { runGemini } = await import('./runGemini-BlykpgGG.mjs');
      let startedBy = void 0;
    ...
    try {
      const { runAcp, resolveAcpAgentConfig } = await import('./index-CNIvBVcN.mjs');
      let startedBy = void 0;
      let verbose = false;
      const acpArgs = [];
    ...
    try {
      const { runOpenClaw } = await import('./runOpenClaw-CqWRlIRB.mjs');
      let startedBy = void 0;
      let verbose = false;
      let gatewayUrl;
      let gatewayToken;
...
export { BasePermissionHandler as B, CHANGE_TITLE_INSTRUCTION as C, DEFAULT_GEMINI_MODEL as D, GEMINI_MODEL_ENV as G, MessageQueue2 as M, startHappyServer as a, GEMINI_API_KEY_ENV as b, createSessionMetadata as c, GOOGLE_API_KEY_ENV as d, BaseReasoningProcessor as e, MessageBuffer as f, hashObject as h, initialMachineMetadata as i, notifyDaemonSessionStarted as n, registerKillSessionHandler as r, setupOfflineReconnection as s };
