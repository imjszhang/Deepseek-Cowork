...
const { getPublicKeyAsync, signAsync, utils } = ed__namespace;
...
function getOpenClawDir(homeDir) {
  return node_path.join(homeDir, OPENCLAW_DIR_NAME);
}
...
function writeJsonFile(filePath, data) {
  const dir = node_path.join(filePath, "..");
  ensureDir(dir);
  node_fs.writeFileSync(filePath, JSON.stringify(data, null, 2), "utf-8");
}
function fingerprintPublicKey(publicKey) {
  const hash = node_crypto.createHash("sha256").update(publicKey).digest();
...
async function loadOrCreateDeviceIdentity(homeDir) {
  ...
  const filePath = node_path.join(getOpenClawDir(homeDir), DEVICE_IDENTITY_FILE);
...
async function loadDeviceAuthToken(homeDir) {
  ...
  const filePath = node_path.join(getOpenClawDir(homeDir), DEVICE_AUTH_TOKEN_FILE);
...
async function storeDeviceAuthToken(homeDir, params) {
  ...
  writeJsonFile(node_path.join(getOpenClawDir(homeDir), DEVICE_AUTH_TOKEN_FILE), stored);
...
class OpenClawBackend {
  ...
  handleEvent(event, payload) {
    ...
    if (state === "tool") {
      ...
      const callId = chatEvent.toolCallId ?? node_crypto.randomUUID();
...
function openclawExec(...args) {
  try {
    return node_child_process.execFileSync("openclaw", args, { timeout: 1e4, encoding: "utf-8", windowsHide: true }).trim();
  } catch {
    return null;
  }
}
...
function resolveConfigPath() {
  ...
  const stateDir = process.env.OPENCLAW_STATE_DIR ?? node_path.join(os.homedir(), ".openclaw");
  return node_path.join(stateDir, "openclaw.json");
}
function queryGatewayToken() {
  try {
    const raw = JSON.parse(node_fs.readFileSync(resolveConfigPath(), "utf-8"));
...
async function runOpenClaw(opts) {
  ...
  api.connectionState.setBackend("openclaw");
  const gatewayConfig = resolveGatewayConfig(opts);
  const log = (msg) => {
    api.logger.debug(`[openclaw] ${msg}`);
    if (verbose) {
  ...
  if (response) {
    try {
      ...
      api.logger.debug("[openclaw] Failed to report session to daemon:", error);
    }
  }
  const sessionManager = new AcpSessionManager.AcpSessionManager();
  const messageQueue = new index.MessageQueue2(() => "");
  ...
  async function handleAbort() {
    ...
    try {
      ...
      api.logger.debug("[openclaw] Abort failed:", error);
    }
    inTurn = false;
    thinking = false;
  ...
  try {
    ...
    try {
      ...
      api.logger.debug("[openclaw] Session close failed:", error);
    }
  }
}

exports.runOpenClaw = runOpenClaw;
