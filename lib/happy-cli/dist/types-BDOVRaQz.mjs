...
import { randomBytes, createCipheriv, createDecipheriv, randomUUID, createHash as createHash$1, createHmac } from 'node:crypto';
...
import { dirname, resolve, join as join$1, sep } from 'path';
...
async function listDaemonLogFiles(limit = 50) {
  try {
    ...
    try {
      const { readDaemonState } = await import('./persistence-tjdFxr4R.mjs');
...
function run(args, options) {
  ...
  return new Promise((resolve2, reject) => {
    const child = spawn$1(binaryPath, args, {
      stdio: ["pipe", "pipe", "pipe"],
      cwd: options?.cwd,
      windowsHide: true,
      env: {
    ...
    try {
      ...
      const directoryEntries = await Promise.all(
        entries.map(async (entry) => {
          const fullPath = join$1(directoryPath, entry.name);
          let type = "other";
          let size;
          let modified;
  ...
  if (message.type === "user") {
    if (typeof message.message.content === "string") {
      if (message.isSidechain) {
        ...
        envelopes.push(createEnvelope("agent", { t: "text", text: message.message.content }, { turn: turnId2, subagent }));
      } else {
...
export { ApiClient as A, InvalidateSync as I, RawJSONLinesSchema as R, ApiSessionClient as a, connectionState as b, configuration as c, packageJson as d, encodeBase64 as e, delay as f, AsyncLock as g, encodeBase64Url as h, decodeBase64 as i, getLocalHappyAgentCredentialPath as j, decryptWithDataKey as k, logger as l, decryptLegacy as m, detectResumeSupport as n, detectCLIAvailability as o, projectPath as p, decrypt as q, readLocalHappyAgentCredentials as r, startOfflineReconnection as s, getLatestDaemonLog as t, api as u };
