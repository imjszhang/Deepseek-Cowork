    ...
    for (const testPath of possiblePaths) {
        ...
        try {
            if (fs.existsSync(checkPath)) {
                logInfo(`Found happy CLI at: ${testPath}`);
                return testPath;
            }
        } catch (e) {
            // 忽略检查错误
...
class DaemonManager extends EventEmitter {
    ...
    _buildDaemonEnv() {
        ...
        try {
            ...
            if (claudeEnv) {
                ...
                if (claudeEnv.ANTHROPIC_DEFAULT_OPUS_MODEL) {
                    env.ANTHROPIC_DEFAULT_OPUS_MODEL = claudeEnv.ANTHROPIC_DEFAULT_OPUS_MODEL;
                    logDebug(`Injecting ANTHROPIC_DEFAULT_OPUS_MODEL: ${claudeEnv.ANTHROPIC_DEFAULT_OPUS_MODEL}`);
                }
                if (claudeEnv.ANTHROPIC_DEFAULT_SONNET_MODEL) {
                    env.ANTHROPIC_DEFAULT_SONNET_MODEL = claudeEnv.ANTHROPIC_DEFAULT_SONNET_MODEL;
                    logDebug(`Injecting ANTHROPIC_DEFAULT_SONNET_MODEL: ${claudeEnv.ANTHROPIC_DEFAULT_SONNET_MODEL}`);
                }
                if (claudeEnv.ANTHROPIC_DEFAULT_HAIKU_MODEL) {
                    env.ANTHROPIC_DEFAULT_HAIKU_MODEL = claudeEnv.ANTHROPIC_DEFAULT_HAIKU_MODEL;
                    logDebug(`Injecting ANTHROPIC_DEFAULT_HAIKU_MODEL: ${claudeEnv.ANTHROPIC_DEFAULT_HAIKU_MODEL}`);
                }
                if (claudeEnv.CLAUDE_CODE_SUBAGENT_MODEL) {
                    env.CLAUDE_CODE_SUBAGENT_MODEL = claudeEnv.CLAUDE_CODE_SUBAGENT_MODEL;
                    logDebug(`Injecting CLAUDE_CODE_SUBAGENT_MODEL: ${claudeEnv.CLAUDE_CODE_SUBAGENT_MODEL}`);
                }
                if (claudeEnv.CLAUDE_CODE_EFFORT_LEVEL) {
                    env.CLAUDE_CODE_EFFORT_LEVEL = claudeEnv.CLAUDE_CODE_EFFORT_LEVEL;
                    logDebug(`Injecting CLAUDE_CODE_EFFORT_LEVEL: ${claudeEnv.CLAUDE_CODE_EFFORT_LEVEL}`);
                }
     ...
     * @private
     */
    async _checkApiServerHealth(timeoutMs = 5000) {
        // 从配置获取服务器地址，默认使用 api.deepseek-cowork.com
        const serverUrl = process.env.HAPPY_SERVER_URL || 'https://api.deepseek-cowork.com';
        ...
        try {
            ...
            if (response.ok) {
                logDebug(`API server health check passed: ${healthUrl}`);
                return true;
            }
            
            logDebug(`API server health check failed: HTTP ${response.status}`);
            return false;
        } catch (e) {
            logDebug(`API server health check failed: ${e.message}`);
            return false;
        }
    }
    ...
    async _doStartDaemon(timeout) {
        ...
        return new Promise((resolve, reject) => {
            ...
            // 注意：
            // 1. 不使用 shell: true，避免 Windows 上打开新命令行窗口
            // 2. Windows 上使用 stdio: 'ignore'，因为 pipe 与 windowsHide 冲突会导致控制台窗口弹出
            // 3. Windows 上 detached + windowsHide 组合可能有短暂窗口闪烁（Node.js 已知限制）
            ...
            // 捕获 stdout 输出（daemon 可能输出启动信息）- 仅非 Windows
            if (!isWindows && daemonProcess.stdout) {
            ...
            // 捕获 stderr 输出（用于诊断错误）- 仅非 Windows
            if (!isWindows && daemonProcess.stderr) {
            ...
            const checkInterval = setInterval(async () => {
                ...
                if (!isProcessRunning(state.pid)) {
                    // 进程不存在，可能是启动后崩溃了
                    // 立即清理孤儿 session 进程，避免资源泄露
                    logInfo('Daemon process crashed, cleaning up orphaned sessions...');
                    self._killOrphanedSessionProcesses().catch(e => {
                        logWarn(`Failed to cleanup orphaned sessions: ${e.message}`);
                    });
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                if (attempt > 0) {
                    ...
                    // 检查 API 服务器健康状态，根据结果调整等待时间
                    const apiHealthy = await this._checkApiServerHealth();
                    if (apiHealthy) {
                        // API 服务器正常，使用标准等待时间
                        logInfo('API server is reachable, waiting 2s before retry...');
                        await sleep(2000);
                    } else {
                        // API 服务器不可达，可能是网络问题，等待更长时间
                        logWarn('API server not reachable, waiting 5s before retry (network issue suspected)...');
                        await sleep(5000);
                    }
                    
                    // 再次清理可能的残留状态
                    await this._cleanupStaleState();
                }
                ...
                // 如果错误信息包含 timeout，可能是网络超时导致
                if (error.message && error.message.includes('timeout')) {
                    logWarn('Timeout detected, this may be caused by network issues with API server');
                }
            }
        }
     ...
     * 支持 Windows (PowerShell/WMIC/cmd)、Linux、macOS
     * @private
     * @returns {Promise<void>}
     */
    async _killOrphanedSessionProcesses() {
        ...
        try {
            logInfo('DaemonManager: Checking for orphaned session processes...');
            
            if (isWindows) {
                // Windows: 尝试多种方法查找进程
                const pids = await this._findOrphanedProcessesWindows(execAsync);
                
                if (pids.length === 0) {
                    logInfo('DaemonManager: No orphaned session processes found');
                    return;
                }
                
                logInfo(`DaemonManager: Found ${pids.length} orphaned session processes, terminating...`);
                
                // 使用 taskkill 终止进程（通用于所有 Windows 版本）
                for (const pid of pids) {
                    try {
                        await execAsync(`taskkill /PID ${pid} /F /T`, { timeout: 5000 });
                        logInfo(`DaemonManager: Terminated orphaned process PID: ${pid}`);
                    } catch (e) {
                        // taskkill 失败可能是进程已退出
                        logDebug(`DaemonManager: taskkill PID ${pid}: ${e.message}`);
                    }
                }
                
                logInfo('DaemonManager: Orphaned session cleanup complete');
                
            } else {
                // Unix/Mac/Linux: 尝试多种方法查找进程
                const pids = await this._findOrphanedProcessesUnix(execAsync);
                ...
                for (const pid of pids) {
                    try {
                        ...
                        // ESRCH 表示进程不存在，忽略
                        if (e.code !== 'ESRCH') {
                            logDebug(`DaemonManager: kill PID ${pid}: ${e.message}`);
                        }
                    }
                }
            ...
            logWarn(`DaemonManager: Error checking orphaned processes: ${error.message}`);
        }
    }

    /**
     * Windows 下查找孤儿进程
     * 按优先级尝试: PowerShell -> WMIC -> cmd findstr
     * @private
     * @param {Function} execAsync promisified exec
     * @returns {Promise<string[]>} PID 列表
     */
    async _findOrphanedProcessesWindows(execAsync) {
        // 方法 1: PowerShell (推荐，Windows 10/11 都支持)
        try {
            ...
            if (pids.length > 0) {
                logDebug('DaemonManager: Found processes via PowerShell Get-CimInstance');
                return pids;
            }
        } catch (e) {
            logDebug(`DaemonManager: PowerShell Get-CimInstance failed: ${e.message}`);
        }

        // 方法 2: WMIC (旧版 Windows，Windows 11 已弃用但仍可能可用)
        try {
            const { stdout } = await execAsync(
            ...
            if (pids.length > 0) {
                logDebug('DaemonManager: Found processes via WMIC');
                return pids;
            }
        } catch (e) {
            logDebug(`DaemonManager: WMIC failed: ${e.message}`);
        }

        // 方法 3: tasklist + findstr (最基础的方法，作为后备)
        try {
            // 使用 PowerShell 作为 shell 来处理管道，避免 cmd 的引号问题
            const { stdout } = await execAsync(
                'powershell -NoProfile -Command "tasklist /V /FO CSV | Select-String \'started-by daemon\'"',
                { timeout: 10000, windowsHide: true }
            );
            // 从 CSV 输出中提取 PID（第二列）
            const pids = [];
            ...
            if (pids.length > 0) {
                logDebug('DaemonManager: Found processes via tasklist');
                return pids;
            }
        } catch (e) {
            logDebug(`DaemonManager: tasklist failed: ${e.message}`);
        }
    ...
    /**
     * Unix/Mac/Linux 下查找孤儿进程
     * 按优先级尝试: pgrep -> ps + grep
     * @private
     * @param {Function} execAsync promisified exec
     * @returns {Promise<string[]>} PID 列表
     */
    async _findOrphanedProcessesUnix(execAsync) {
        // 方法 1: pgrep (大多数 Linux 和 macOS 都有)
        try {
            const { stdout } = await execAsync(
            ...
            if (pids.length > 0) {
                logDebug('DaemonManager: Found processes via pgrep');
                return pids;
            }
        } catch (e) {
            // pgrep 找不到进程时返回退出码 1，这是正常的
            if (e.code !== 1) {
                logDebug(`DaemonManager: pgrep failed: ${e.message}`);
            }
        }

        // 方法 2: ps + grep (更通用，适用于没有 pgrep 的系统如 Alpine)
        try {
            // 使用 ps aux 并 grep，排除 grep 自身
            const { stdout } = await execAsync(
            ...
            const pids = stdout.trim().split(/\n/).filter(Boolean);
            if (pids.length > 0) {
                logDebug('DaemonManager: Found processes via ps + grep');
                return pids;
            }
        } catch (e) {
            logDebug(`DaemonManager: ps + grep failed: ${e.message}`);
        }

        // 方法 3: 纯 ps 命令 (适用于精简系统，可能没有 awk)
        try {
            const { stdout } = await execAsync(
                "ps -eo pid,args | grep -- '--started-by daemon' | grep -v grep",
                { timeout: 5000, shell: '/bin/sh' }
            );
            const pids = [];
            ...
            if (pids.length > 0) {
                logDebug('DaemonManager: Found processes via ps -eo');
                return pids;
            }
        } catch (e) {
            logDebug(`DaemonManager: ps -eo failed: ${e.message}`);
        }
