...
const memoryConfig = require('../memory-manager/config');
...
class HappyServiceClass extends EventEmitter {
    ...
    async reconnectSession(name) {
        ...
        return await this.sessionManager.reconnectSession(name);
    }
    ...
    async connectToSession(sessionName = 'main', options = {}) {
        ...
        // 如果 session 不存在（可能是账户变更后清理了），尝试重新创建
        if (!sessionId) {
            if (secretChanged || !this.sessionManager.sessions[sessionName]) {
                logInfo(`[HappyService] Session "${sessionName}" not found, creating new session...`);
                try {
                    // 获取工作目录配置
                    const workDirs = this.sessionManager.getWorkDirsFromConfig();
                    const workDirConfig = workDirs.find(w => w.name === sessionName);
                    const workDir = workDirConfig?.path || this._options.baseDir || process.cwd();
                    
                    // 创建新 session
                    const sessionInfo = await this.sessionManager.createSession(sessionName, workDir, { allowReuse: false });
                    sessionId = sessionInfo?.sessionId;
                    
                    if (sessionId) {
                        logInfo(`[HappyService] Created new session: ${sessionId}`);
                        // 保存状态
                        this.sessionManager.saveStateFile();
                    }
                } catch (e) {
        ...
        try {
            ...
            // 初始化记忆管理器
            const workDir = this._options.baseDir || process.cwd();
            await this._initializeMemoryManager(workDir);
    ...
    _setupClientEventForwarding() {
        ...
        this.happyClient.on('daemon:sessionSpawned', (event) => {
            ...
            // 更新 sessions 映射
            this.sessionManager.sessions[sessionName] = {
                sessionId,
            ...
            // 更新 workDirIndex 映射
            const normalizedPath = this.sessionManager._normalizePathForIndex(workDir);
            this.sessionManager.workDirIndex[normalizedPath] = sessionName;
    ...
    _handleSyncMessage(event) {
        ...
        if (!this._isClearCommand) {
            ...
            if (usageData) {
                ...
                // 更新 MemoryManager 预算
                if (this.memoryManager) {
                    this.memoryManager.updateBudget(usageData);
                }
     ...
     * @private
     */
    async _initializeMemoryManager(workDir) {
        // 检查是否启用记忆功能
        if (this._options.memory?.enabled === false) {
            logInfo('[HappyService] Memory manager disabled by config');
            return;
        }
        ...
        try {
            ...
            this.memoryManager = new MemoryManager({
                workDir,
                dataDir: this._options.memory?.dataDir,    // 可选：自定义数据目录
                skillDir: this._options.memory?.skillDir,  // 可选：自定义技能目录
                contextLimit: this._options.contextLimit || 200000,
                autoSave: this._options.memory?.autoSave
            });
            ...
            if (success) {
                logInfo('[HappyService] MemoryManager initialized');
                
                // 监听自动保存事件（上下文阈值触发）
                this.memoryManager.on('autoSave:threshold', async (data) => {
                    logInfo(`[HappyService] Auto-save triggered by context threshold: ${(data.usageRatio * 100).toFixed(1)}%`);
                    // 简化：直接发送"保存记忆"，让 AI 根据 SKILL.md 处理
                    if (this.happyClient && this._clientConnected) {
                        this.happyClient.sendMessage('保存记忆');
                    }
                });
                
                this.memoryManager.on('memory:saved', (data) => {
                    logInfo(`[HappyService] Memory saved: ${data.name}`);
                    this.emit('memory:saved', data);
                });
            } else {
                logWarn('[HappyService] MemoryManager initialization failed, memory features disabled');
                this.memoryManager = null;
            }
        } catch (error) {
            logError(`[HappyService] Failed to initialize MemoryManager: ${error.message}`);
            this.memoryManager = null;
        }
    }
    ...
    async sendMessage(text) {
        ...
        try {
            ...
            if (this.memoryManager) {
                const memoryContext = this.memoryManager.getContextInjection(text);
                if (memoryContext) {
                    fullSystemPrompt = memoryContext + systemPrompt;
                }
            }
            
            // 发送消息（附加系统提示词，包含记忆上下文）
    ...
    async restartDaemon(newOptions = {}) {
        ...
        try {
            ...
            if (newOptions && Object.keys(newOptions).length > 0) {
                ...
                // 如果提供了新的 anonId，更新当前 anonId（避免后续 connectToSession 检测到"变更"并重复清理）
