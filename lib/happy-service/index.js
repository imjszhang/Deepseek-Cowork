...
class HappyServiceClass extends EventEmitter {
    constructor() {
        ...
        // processing 状态看门狗，避免底层会话静默卡死后一直无法继续发送
        this._processingWatchdogTimeout = null;
        this._processingWatchdogMs = 120000;
    }
    ...
    async initialize(options = {}) {
        ...
        try {
            ...
            if (!hasSecret) {
                ...
                // 转发 SessionManager 的单个 session 状态变化事件（轻量级）
                this.sessionManager.on('session:statusChanged', (data) => {
     ...
     * @returns {Object} { [sessionId]: { messages: [...], lastUpdated } }
     */
    getMultiSessionMessages(sessionIds, limit = 5) {
        const result = {};
        ...
        for (const sessionId of sessionIds) {
            ...
            const messages = MessageStore.getMessages(sessionId, limit);
            
            // 截断消息文本，只返回预览所需的字段
            const previewMessages = messages.map(msg => ({
                role: msg.role,
                text: this._truncateText(msg.text || this._extractMessageText(msg.content), 150),
        ...
        logDebug(`[HappyService] getMultiSessionMessages: ${sessionIds.length} sessions, limit=${limit}`);
        return result;
    }
    ...
    _setupClientEventForwarding() {
        ...
        // agentState 更新事件（权限请求）
        // 当收到权限请求时，转发到前端，让前端显示确认对话框
        this.happyClient.on('agentStateUpdated', (event) => {
            this._handleAgentStateUpdated(event);
        });
        ...
        // 流事件 - 结束
        // 使用延迟更新，确保消息先被处理完（避免 ready 事件比消息先到达前端）
        this.happyClient.on('conversation:streamEnded', () => {
            this._scheduleReadyStatus('streamEnded');
        });
        ...
        this.happyClient.on('conversation:eventStatus', (event) => {
            ...
            if (eventType === 'message') {
                this._handleRuntimeEventMessage(event);
                return;
            }
            
            // 只处理已知的状态类型，忽略其他类型（如 message, switch 等）
            const knownStatuses = ['ready', 'idle', 'processing', 'thinking', 'waiting', 'disconnected'];
            if (!knownStatuses.includes(eventType)) {
                logDebug(`[HappyService] Ignoring unknown eventStatus: ${eventType}`);
                return;
            }
            
            // 防抖机制：收到 ready 事件后，短时间内忽略 processing 事件
            // 这是因为 ready 事件和最后一条消息可能几乎同时到达，导致状态竞态
            if (eventType === 'ready' || eventType === 'idle') {
                // 使用延迟更新，确保消息先被处理完
                this._scheduleReadyStatus('eventStatus');
            ...
            } else if (eventType === 'processing' || eventType === 'thinking' || eventType === 'waiting') {
                // 取消待执行的 ready 状态更新（新的 processing 事件说明还在工作中）
                ...
                this._updateEventStatus(eventType);
            }
        });
    ...
    /**
     * 将底层 runtime event message 透传给 UI，避免只更新状态而吞掉错误文本。
     * @param {Object} event ConversationManager 事件
     * @private
     */
    _handleRuntimeEventMessage(event = {}) {
        const eventData = event.eventData || {};
        const text = eventData.message || eventData.text;
        if (!text) {
            return;
        }
        ...
        logWarn(`[HappyService] Runtime event message: ${text}`);
        this.emit('happy:error', {
            type: 'runtime_event',
            message: text,
            sessionId,
    ...
    _handleSyncMessage(event) {
        ...
        const text = this._extractMessageText(content);
        const messageData = {
        ...
        // 发送原始消息事件（前端 Reducer 处理）。外部模块异常不应中断消息链路。
        try {
            this.emit('happy:message', messageData);
        } catch (error) {
            logWarn(`[HappyService] happy:message listener failed: ${error.message}`);
        }
        ...
        if (role === 'user') {
            ...
            // AI 消息后设置静默检测定时器
            // 如果 3 秒内没有新消息，自动触发 ready 状态
            this._scheduleSilenceReady();
        } else if (role === 'session') {
            if (content?.ev?.t === 'turn-end') {
                this._scheduleReadyStatus('session-turn-end');
            } else if (content?.ev?.t === 'text' && !content.ev.thinking) {
                this._scheduleSilenceReady();
            }
        }
     ...
     * 同时更新该 session 的处理状态（用于 SessionHub 卡片显示）
     * @param {Object} event 消息事件
     * @private
     */
    _handleOtherSessionMessage(event) {
        ...
        // 触发消息添加事件（供看板实时更新）
        const messageText = this._extractMessageText(content);
        this.emit('message:added', {
            sessionId,
            message: {
                role: message.role,
                text: this._truncateText(messageText, 150),
                timestamp: message.timestamp
            }
        });
        
        // 更新该 session 的处理状态（用于 SessionHub 卡片显示）
        this._updateOtherSessionStatus(sessionId, role);
        
        logDebug(`[HappyService] Stored message from other session: ${sessionId.substring(0, 8)}..., role=${message.role}`);
    }
    
    /**
     * 处理 agentState 更新事件（权限请求）
     * 转发到前端，让前端可以显示权限确认对话框
     * @param {Object} event agentState 更新事件
     * @param {string} event.sessionId session ID
     * @param {Object} event.agentState 解密后的 agentState
     * @param {number} event.version agentState 版本
     * @param {boolean} event.hasNewRequests 是否有新的权限请求
     * @private
     */
    _handleAgentStateUpdated(event) {
        const { sessionId, agentState, version, hasNewRequests } = event;
        
        // 只处理当前 session 的 agentState 更新
        if (!sessionId || sessionId !== this.happyClient?.currentSessionId) {
            return;
        }
        
        logInfo(`[HappyService] AgentState updated: version=${version}, hasNewRequests=${hasNewRequests}`);
        
        if (this._options.debug && agentState) {
            logDebug(`[HappyService] AgentState requests: ${JSON.stringify(Object.keys(agentState.requests || {}))}`);
            logDebug(`[HappyService] AgentState completedRequests: ${JSON.stringify(Object.keys(agentState.completedRequests || {}))}`);
        }
        
        // 发出 agentState 更新事件到前端
        this.emit('happy:agentState', {
            sessionId,
            agentState,
            version,
            hasNewRequests,
     ...
     */
    getAgentState() {
        return this.happyClient?.getAgentState() || null;
    }
    
    /**
     * 更新其他 session 的处理状态
     * - 收到 user 消息时：立即设为 processing
     * - 收到 agent 消息时：延迟 2 秒后检查，如果没有新消息则恢复为 idle
     * @param {string} sessionId session ID
     * @param {string} role 消息角色 (user, agent)
     * @private
     */
    _updateOtherSessionStatus(sessionId, role) {
        if (!this.sessionManager) return;
        
        // 初始化定时器 Map
        if (!this._otherSessionStatusTimers) {
            this._otherSessionStatusTimers = new Map();
        }
        
        // 清除之前的定时器
        const existingTimer = this._otherSessionStatusTimers.get(sessionId);
        if (existingTimer) {
            clearTimeout(existingTimer);
            this._otherSessionStatusTimers.delete(sessionId);
        }
        
        if (role === 'user') {
            // 用户消息：立即设为 processing
            logDebug(`[HappyService] Other session ${sessionId.substring(0, 8)}... user message, setting processing`);
            this.sessionManager.updateSessionEventStatus(sessionId, 'processing');
        } else if (role === 'agent') {
            // Agent 消息：延迟 3 秒后恢复为 idle（如果没有新消息）
            logDebug(`[HappyService] Other session ${sessionId.substring(0, 8)}... agent message, scheduling idle`);
            const timer = setTimeout(() => {
                this._otherSessionStatusTimers.delete(sessionId);
                logDebug(`[HappyService] Other session ${sessionId.substring(0, 8)}... timeout, setting ready`);
                this.sessionManager.updateSessionEventStatus(sessionId, 'ready');
            }, 3000);
            this._otherSessionStatusTimers.set(sessionId, timer);
        }
    }
    ...
    _extractMessageText(content) {
        ...
        // 新版 happy session protocol envelope
        if (content.ev) {
            if (content.ev.t === 'text') {
                return content.ev.text || '';
            }
            if (content.ev.t === 'turn-start') {
                return '[任务开始]';
            }
            if (content.ev.t === 'turn-end') {
                return `[任务结束: ${content.ev.status || 'completed'}]`;
            }
            return `[事件: ${content.ev.t || 'unknown'}]`;
        }
    ...
    _updateEventStatus(status) {
        const oldStatus = this._eventStatus;
        logDebug(`[HappyService] _updateEventStatus called: ${oldStatus} -> ${status}`);
        ...
        if (oldStatus !== status) {
            ...
            logDebug(`[HappyService] Emitting happy:eventStatus: ${status}`);
            this.emit('happy:eventStatus', {
                eventType: status,
                timestamp: new Date().toISOString()
            });
            
            // 同步更新 SessionManager 中当前 session 的状态
            // 这样 SessionHub 可以通过 session:stateUpdated 事件获取所有 session 的实时状态
            const currentSessionId = this.happyClient?.currentSessionId;
            if (currentSessionId && this.sessionManager) {
                logDebug(`[HappyService] Updating SessionManager status for ${currentSessionId}: ${status}`);
                this.sessionManager.updateSessionEventStatus(currentSessionId, status);
            }
        } else {
            logDebug(`[HappyService] Status unchanged, skipping update`);
            if (status === 'processing') {
                this._startProcessingWatchdog();
            }
        }
    ...
    /**
     * 启动 processing 状态看门狗。
     * 如果底层没有返回 streamEnded/ready/error，主动恢复状态并通知 UI。
     * @private
     */
    _startProcessingWatchdog() {
        ...
        const sessionId = this.happyClient?.currentSessionId;
        this._processingWatchdogTimeout = setTimeout(() => {
            this._processingWatchdogTimeout = null;
            
            if (this._eventStatus !== 'processing') {
                return;
            }
            
            const message = 'AI 会话长时间无响应，本轮请求已自动结束，请重试。';
            logWarn(`[HappyService] Processing watchdog timeout after ${this._processingWatchdogMs}ms, session=${sessionId || 'unknown'}`);
    ...
    _clearProcessingWatchdog() {
        if (this._processingWatchdogTimeout) {
            clearTimeout(this._processingWatchdogTimeout);
            this._processingWatchdogTimeout = null;
        }
    }
    ...
    /**
     * 延迟更新 ready 状态
     * 确保消息先被处理完再更新状态，避免 ready 事件比消息先到达前端
     * @param {string} source 触发源（用于调试）
     * @private
     */
    _scheduleReadyStatus(source) {
        // 如果已有待执行的 ready 更新，不重复调度
        if (this._pendingReadyTimeout) {
            return;
        }
        
        // 延迟 100ms 更新 ready 状态，给消息处理留出时间
        this._pendingReadyTimeout = setTimeout(() => {
            this._pendingReadyTimeout = null;
            this._lastReadyTime = Date.now();
            this._updateEventStatus('ready');
            logDebug(`[HappyService] Ready status updated (source: ${source})`);
        }, 100);
    }
    
    /**
     * 取消待执行的 ready 状态更新
     * 当收到新的 processing 事件时调用
     * @private
     */
    _cancelPendingReady() {
        if (this._pendingReadyTimeout) {
            clearTimeout(this._pendingReadyTimeout);
            this._pendingReadyTimeout = null;
            logDebug('[HappyService] Cancelled pending ready status update');
        }
        // 同时取消静默检测定时器
        if (this._silenceReadyTimeout) {
            clearTimeout(this._silenceReadyTimeout);
            this._silenceReadyTimeout = null;
        }
    }
    
    /**
     * 设置静默检测定时器
     * 当收到 AI 消息后，如果 3 秒内没有新消息，自动触发 ready 状态
     * 这是一个备用机制，当后端没有发送 ready 事件时使用
     * @private
     */
    _scheduleSilenceReady() {
        // 清除之前的静默检测定时器
        if (this._silenceReadyTimeout) {
            clearTimeout(this._silenceReadyTimeout);
        }
        ...
        // 设置新的静默检测定时器（3 秒）
        this._silenceReadyTimeout = setTimeout(() => {
            this._silenceReadyTimeout = null;
            // 只有当状态仍然是 processing 时才触发
            if (this._eventStatus === 'processing') {
                logDebug('[HappyService] Silence timeout, triggering ready status');
                this._scheduleReadyStatus('silence_timeout');
            }
        }, 3000);
    }
    ...
    _addToHistory(message) {
        ...
        if (sessionId) {
            ...
            // 触发消息添加事件（供看板实时更新）
            const messageText = message.text || this._extractMessageText(message.content);
            this.emit('message:added', {
                sessionId,
                message: {
                    role: message.role,
    ...
    async abortSession(sessionId) {
        ...
        const isCurrentSession = targetSessionId === this.happyClient.currentSessionId;
        
        try {
            logInfo(`Aborting session ${targetSessionId} (current: ${isCurrentSession})`);
            
            await this.happyClient.abortSession(targetSessionId);
            
            // 更新被中止 session 的状态
            if (isCurrentSession) {
                // 当前 session：清除静默检测定时器并更新状态
                if (this._silenceReadyTimeout) {
                    clearTimeout(this._silenceReadyTimeout);
                    this._silenceReadyTimeout = null;
                }
                ...
                // 其他 session：清除该 session 的静默检测定时器
                if (this._otherSessionStatusTimers?.has(targetSessionId)) {
                    clearTimeout(this._otherSessionStatusTimers.get(targetSessionId));
                    this._otherSessionStatusTimers.delete(targetSessionId);
                }
                // 更新 SessionManager 中的状态
                if (this.sessionManager) {
                    this.sessionManager.updateSessionEventStatus(targetSessionId, 'ready');
                }
            }
